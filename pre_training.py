...
def main(args):
    ...
    for epoch in range(0, args.epochs):
        ...
        train_stats = train_one_epoch(
            args, model, train_dataloader, optimizer, epoch, model_without_ddp=model_without_ddp
        )

        # --- SAVE CKPT (rank 0 only) ---
        try:
            import utils
        ...
        if is_main:
            import os
            save_dir = os.path.join(args.output_dir, f"epoch_{epoch:03d}")
            os.makedirs(save_dir, exist_ok=True)
            # Uni-Sign typically keeps the MT5 under .mt5_model; fall back to the bare model
            to_save = getattr(model_without_ddp, "mt5_model", model_without_ddp)
            # safe_serialization=True -> .safetensors; HF-compatible
            to_save.save_pretrained(save_dir, safe_serialization=True)
            with open(os.path.join(args.output_dir, "LAST_EPOCH"), "w") as f:
                f.write(str(epoch))
        # --- /SAVE CKPT ---
